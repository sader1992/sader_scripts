...
ACMD_FUNC(petstats)
{
	...
	pet_id = search_petDB_index(pd->petDB->class_, PET_CLASS);
	...
	sprintf(mobname, "Mob Name - %s", pet_db[pet_id].name);//pet_db_ptr->name);
	output_table[8].format = mobname;
	output_table[9].value = pet_db[pet_id].class_;
	output_table[10].value = pet_db[pet_id].EggID;
	output_table[11].value = pet_db[pet_id].FoodID;
	output_table[12].value = pet_db[pet_id].AcceID;
